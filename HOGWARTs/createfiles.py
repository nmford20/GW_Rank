...
def createtxt(dataf, finalgalnamelist, finaldictsorted,graceid,prelim,levelsper,d,ccc):
    ...
    tfile.close()
    return   
def createjsonfile(jsonlist,graceid,prelim,levelsper,d,outputdir):
    csv=str('[')
    ...
    filepath = outputdir+graceid+prelim+str(levelsper[d])+".json"
...
def createasciifile(jsonlist2,graceid,prelim,levelsper,d,outputdir):
    ...
    filepath = outputdir+graceid+prelim+str(levelsper[d])+".dat"
    pathlib.Path(filepath).parents[0].mkdir(parents=True, exist_ok=True)
    f = open(outputdir+graceid+prelim+str(levelsper[d])+".dat", "w")
